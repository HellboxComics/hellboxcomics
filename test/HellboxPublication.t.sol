...
contract HellboxPublicationTest {
    ...
    function testNative216ShapeFreezesExpectedConfiguration() public {
        ...
        HellboxPublication publication =
            new HellboxPublication(config, commitments, expectedDigest);
        ...
        require(
            keccak256(bytes(publication.name())) ==
                keccak256(bytes("Hellbox Native Issue #1")),
            "collection name"
        );
        require(
            keccak256(bytes(publication.symbol())) ==
                keccak256(bytes("HELL001")),
            "collection symbol"
        );
        ...
        require(
            publication.immediateCreatorRecipient() == CREATOR,
            "creator recipient"
        );
        ...
        require(
            publication.publisherAuthority() == PUBLISHER,
            "publisher authority"
        );
        ...
        require(
            publication.packageDigest() == commitments.packageDigest,
            "package digest"
        );
        require(
            publication.commitmentsDigest() ==
                _commitmentsDigest(commitments),
            "commitments digest"
        );
        require(
        ...
        require(
            publication.frozenAtBlock() == block.number,
            "freeze block"
        );
        require(
            publication.frozenAtTimestamp() == block.timestamp,
            "freeze timestamp"
        );
    }
    ...
    function testCommitmentsDigestMatchesIndependentEncoding() public {
        ...
        bytes32 expectedReleaseDigest = _releaseDigest(
            block.chainid,
            address(this),
            config,
            commitments
        ...
        require(
            contractDigest == keccak256(abi.encode(commitments)),
    ...
    function testReleaseDigestMatchesIndependentEncoding() public {
        ...
        bytes32 expected = _releaseDigest(
            block.chainid,
            address(this),
            config,
            commitments
        ...
        HellboxPublication publication =
            new HellboxPublication(config, commitments, expected);

        bytes32 helperResult = publication.computeReleaseConfigDigest(
            block.chainid,
            address(this),
            config,
            commitments
        ...
        require(helperResult == expected, "release encoding mismatch");
        require(
            publication.releaseConfigDigest() == expected,
            "constructor digest mismatch"
        );
    }
    ...
    function testERC721AndRoyaltyInterfacesAreSupported() public {
        ...
        require(
            publication.supportsInterface(0x01ffc9a7),
            "ERC165 missing"
        );
        require(
            publication.supportsInterface(0x80ac58cd),
            "ERC721 missing"
        );
        require(
            publication.supportsInterface(0x2a55205a),
            "ERC2981 missing"
        );
    }
    ...
    function testDefaultRoyaltyIsConfigured() public {
        ...
        (address receiver, uint256 amount) =
            publication.royaltyInfo(1, 1 ether);
    ...
    function testPublicationHasNoGenericOwnableOwner() public {
        ...
        (bool success, bytes memory returnData) =
            address(publication).staticcall(
    ...
    function testRejectsInvalidPublicationKeys() public {
        ...
        for (uint256 i = 0; i < invalidKeys.length; ++i) {
            ...
            _expectValidationRevert(
                config,
                commitments,
                HellboxPublication.InvalidPublicationKey.selector
            );
        }
    }
    ...
    function testRejectsInvalidCollectionNameAndSymbol() public {
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidCollectionNameLength.selector
        );
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidCollectionSymbol.selector
        );
    }
    ...
    function testRejectsInvalidSupplyAndCaps() public {
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidMaxSupply.selector
        );
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidPrimaryLifetimeCap.selector
        );
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidMaxPerTransaction.selector
        );
    }
    ...
    function testRejectsInvalidCreatorAndTailRules() public {
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidCreatorAllocation.selector
        );
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidImmediateCreatorRecipient.selector
        );
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidTailRecipient.selector
        );
    }
    ...
    function testRejectsInvalidRoyaltyRules() public {
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidRoyaltyConfiguration.selector
        );
    ...
    function testRejectsMissingPublisherAuthority() public {
        ...
        _expectValidationRevert(
            config,
            _commitments(),
            HellboxPublication.InvalidPublisherAuthority.selector
        );
    }
    ...
    function testRejectsInvalidCapabilityDependencies() public {
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.InvalidCapabilityConfiguration.selector
        );
    ...
    function testRejectsMissingRequiredCommitments() public {
        ...
        _expectValidationRevert(
            config,
            commitments,
            HellboxPublication.MissingRequiredCommitment.selector
        );
