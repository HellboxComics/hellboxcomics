...
contract ReentrantCreatorReceiver is IERC721Receiver {
    ...
    function onERC721Received(
        ...
        if (!reentryAttempted) {
            ...
            (beginCallSucceeded, beginData) = publication.call(
                abi.encodeWithSelector(
                    HellboxPublication.beginCreatorInitialization.selector
                )
            );
            ...
            (fulfillCallSucceeded, fulfillData) = publication.call(
                abi.encodeWithSelector(
                    HellboxPublication.fulfillNextRandomnessRequest.selector,
                    bytes("")
                )
            );
...
contract HellboxProductionPrizeWalletIntegrationTest {
    ...
    bytes32 internal constant MOCK_VERIFIED_RANDOMNESS =
        keccak256("hellbox-real-factory-integration-randomness");

    bytes4 internal constant VERIFY_ROUND_SELECTOR =
        IHellboxRandomnessVerifier.verifyRound.selector;
    ...
    function testRealFactoryProductionFlowReservesFulfillsAndCompletes()
        public
    {
        Deployment memory deployed = _deployProductionPublication(
            "real-factory-production-success",
            CREATOR,
            true
        );

        (uint256 firstRequestId, uint64 firstRound) =
        ...
        for (uint256 index = 0; index < 6; ++index) {
            ...
            (uint256 requestId, uint256 tokenId) =
            ...
            require(
                deployed.publication.ownerOf(tokenId) == CREATOR,
                "creator owner"
            );
        }

        require(
            deployed.publication.immediateCreatorAllocationComplete(),
            "creator allocation incomplete"
        );
        require(deployed.publication.totalPrimaryIssued() == 6, "creator total");
        require(
            deployed.publication.candidatePoolRemaining() == 210,
            "creator candidate total"
        );
        require(
            deployed.publication.nonTailIssuanceRemaining() == 207,
            "creator non-tail total"
        );
        ...
        _assertPendingRequest(
            deployed.publication,
            7,
            HellboxPublication.RandomnessRequestKind.PRIZE_WALLET,
            deployed.wallet
        );

        (uint256 prizeRequestId, uint256 prizeTokenId) =
            _fulfillPending(deployed.publication);

        require(prizeRequestId == 7, "prize request id");
        require(
            deployed.publication.ownerOf(prizeTokenId) == deployed.wallet,
            "prize owner"
        );
        require(
            deployed.publication.prizeWalletTokenId() == prizeTokenId,
            "prize token snapshot"
        );
        require(
            deployed.publication.prizeWalletIssuanceComplete(),
            "prize issuance incomplete"
        );
        require(
            deployed.publication.prizeWalletDepositCompleted(),
            "publication completion missing"
        );
        require(deployed.publication.totalPrimaryIssued() == 7, "seventh mint");
        require(
            deployed.publication.candidatePoolRemaining() == 209,
            "post-prize candidates"
        );
        require(
            deployed.publication.nonTailIssuanceRemaining() == 206,
            "post-prize non-tail"
        );
        require(
        ...
        require(
            deployed.publication.nextPendingRandomnessRequestId() == 0,
            "queue not empty"
        );
    ...
    function testRealFactoryReservationFailureRollsBackSixthCreatorFulfillment()
        public
    {
        Deployment memory deployed = _deployProductionPublication(
            "real-factory-reservation-rollback",
            CREATOR,
            false
        );
        ...
        require(
            deployed.publication.immediateCreatorIssued() == 5,
            "creator count rollback"
        );
        require(
            deployed.publication.balanceOf(CREATOR) == 5,
            "creator balance rollback"
        );
        require(
        ...
        require(
            deployed.publication.nextPendingRandomnessRequestId() == 6,
            "sixth request consumed"
        );
        require(
            !deployed.publication.prizeWalletDepositReserved(),
            "publication reservation leaked"
        );
        require(
            deployed.factory.prizeWalletDepositStateByPublication(
        ...
        uint256 approvedGeneration = deployed.factory.approvePrizeWalletPublication(
            address(deployed.publication)
        );
        require(approvedGeneration == 1, "approval generation");

        (uint256 retriedRequestId, uint256 tokenId) =
            deployed.publication.fulfillNextRandomnessRequest(proof);

        require(retriedRequestId == 6, "request rerolled");
        require(tokenId == 6, "sixth token changed");
        require(
            deployed.publication.ownerOf(6) == CREATOR,
            "sixth creator owner"
        );
        _assertReservedState(deployed);
        require(
            deployed.publication.nextPendingRandomnessRequestId() == 7,
            "prize request missing"
        );
    }

    function testRealFactoryCompletionFailureRollsBackPrizeMintAndRetries()
        public
    {
        Deployment memory deployed = _deployProductionPublication(
            "real-factory-completion-rollback",
            CREATOR,
            true
        );
        ...
        VM.mockCallRevert(
            ...
            abi.encodeWithSelector(
                HellboxPublicationFactory.completePrizeWalletDeposit.selector
            ),
        ...
        require(
            deployed.publication.prizeWalletTokenId() == 0,
            "prize token leaked"
        );
        require(
            deployed.publication.balanceOf(deployed.wallet) == 0,
            "prize mint leaked"
        );
        require(deployed.publication.totalPrimaryIssued() == 6, "supply leaked");
        require(
            deployed.publication.candidatePoolRemaining() == 210,
            "candidate mutation leaked"
        );
        require(
            deployed.publication.nonTailIssuanceRemaining() == 207,
            "non-tail mutation leaked"
        );
        require(
            deployed.publication.randomnessRequestCount() == 7 &&
        ...
        (uint256 retriedRequestId, uint256 prizeTokenId) =
            deployed.publication.fulfillNextRandomnessRequest(proof);
    ...
    function testCreatorReceiverReentrancyCannotDoubleStartOrFulfill()
        public
    {
        ReentrantCreatorReceiver receiver = new ReentrantCreatorReceiver();
        Deployment memory deployed = _deployProductionPublication(
            "real-factory-creator-reentrancy",
            address(receiver),
            true
        );
        ...
        require(
            receiver.beginRevertSelector() ==
                HellboxPublication.RandomnessFulfillmentReentrancy.selector,
            "wrong begin reentry error"
        );
        require(
            receiver.fulfillRevertSelector() ==
                HellboxPublication.RandomnessFulfillmentReentrancy.selector,
            "wrong fulfill reentry error"
        );
        require(receiver.callbackCount() == 6, "creator callback count");
        require(
            deployed.publication.balanceOf(address(receiver)) == 6,
            "creator receiver balance"
        );
        require(deployed.publication.totalPrimaryIssued() == 6, "creator total");
        require(
            deployed.publication.randomnessRequestCount() == 7 &&
    ...
    function testCampaignWalletCodeAfterReservationFailsClosedAndRetries()
        public
    {
        Deployment memory deployed = _deployProductionPublication(
            "real-factory-wallet-code-drift",
            CREATOR,
            true
        );
        ...
        VM.expectPartialRevert(
            HellboxPublication.PrizeWalletRecipientHasCode.selector
        );
        deployed.publication.fulfillNextRandomnessRequest(proof);
    ...
    function _deployProductionPublication(
        ...
        deployed.campaignManifestDigest = keccak256(
            abi.encode(
                "HELLBOX_PRODUCTION_PRIZE_WALLET_INTEGRATION",
                publicationKey
            )
        );
        _activateCampaign(
        ...
        HellboxPublication.ReleaseConfig memory config =
            _nativeConfig(publicationKey, creatorRecipient);
        ...
        HellboxPublication.CommitmentSet memory commitments =
            _commitments(fixedPolicy, birthTraits, randomPolicy);
        ...
        require(
            deployed.factory.isPublication(address(deployed.publication)),
            "publication not registered"
        );
        require(
            deployed.factory.publicationByReleaseDigest(releaseDigest) ==
                address(deployed.publication),
            "release provenance"
        );
        require(
            deployed.publication.factory() == address(deployed.factory),
            "factory binding"
        );
        require(
            deployed.publication.randomnessVerifier() ==
                deployed.factory.randomnessVerifier(),
            "verifier binding"
        );
        require(
        ...
        require(
            deployed.publication.randomnessProviderConfigDigest() ==
                DRAND_PROVIDER_CONFIG_DIGEST,
            "provider binding"
        );
        require(
            deployed.publication.issuanceStateInitialized(),
            "issuance bootstrap"
        );

        if (approvePublication) {
            uint256 approvedGeneration =
                deployed.factory.approvePrizeWalletPublication(
                    address(deployed.publication)
                );
    ...
    function _activateCampaign(
        ...
        require(
            factory.activePrizeWalletManifestDigest() == manifest,
            "active manifest"
        );
    }
    ...
    function _fulfillPending(
        HellboxPublication publication
    ) internal returns (uint256 requestId, uint256 tokenId) {
        ...
        (uint256 fulfilledRequestId, uint256 issuedTokenId) =
    ...
    function _assertReservedState(Deployment memory deployed) internal view {
        require(
            deployed.publication.prizeWalletDepositReserved(),
            "publication not reserved"
        );
        require(
            !deployed.publication.prizeWalletDepositCompleted(),
            "publication completed early"
        );
        require(
            deployed.publication.prizeWalletCampaignGeneration() == 1,
            "publication generation"
        );
        require(
            deployed.publication.prizeWalletCampaignManifestDigest() ==
                deployed.campaignManifestDigest,
            "publication manifest"
        );
        require(
            deployed.publication.prizeWalletRecipient() == deployed.wallet,
            "publication wallet"
        );
        require(
            deployed.factory.prizeWalletDepositGenerationByPublication(
                address(deployed.publication)
            ) == 1,
    ...
    function _nativeConfig(
        string memory publicationKey,
        address creatorRecipient
    ) internal pure returns (HellboxPublication.ReleaseConfig memory config) {
