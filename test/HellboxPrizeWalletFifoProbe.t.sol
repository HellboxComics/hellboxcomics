...
contract DeterministicEvmnetVerifierMock is IHellboxRandomnessVerifier {
    ...
    ) external pure returns (bytes32 randomness) {
        if (proof.length != 64) {
            revert MockProofLength(proof.length);
        }

        (uint64 suppliedRound, bytes32 suppliedRandomness) =
            abi.decode(proof, (uint64, bytes32));
...
contract PrizeWalletPublicationDeployer {
    ...
    function deploy(
        ...
        publication = new PrizeWalletPublicationHarness(
            config,
            commitments,
...
contract PrizeWalletPublicationHarness is HellboxPublication {
    ...
    function candidateTokenAtForTest(
...
contract HellboxPrizeWalletFifoProbeTest {
    ...
    bytes32 internal constant OBSERVED_RANDOMNESS =
    ...
    function testPrizeRequestIsStrictlySeventhAndBindsFutureRound() public {
        DeterministicEvmnetVerifierMock verifier =
            new DeterministicEvmnetVerifierMock();
        (
        ...
        require(
            ...
            "request changed candidates"
        );
        require(
            publication.nonTailIssuanceRemaining() == 207,
            "request changed non-tail"
        );
        require(
            verifier.roundTimestamp(round) >=
                requestedAt + publication.RANDOMNESS_REQUEST_DELAY_SECONDS(),
            "round not future"
        );
        ...
        require(
            kind == HellboxPublication.RandomnessRequestKind.PRIZE_WALLET,
            "request kind"
        );
        ...
        require(
            publication.prizeWalletRecipient() == PRIZE_WALLET,
            "recipient snapshot"
        );
        require(
            publication.prizeWalletRequestId() == requestId,
            "request snapshot"
        );
        require(
            address(deployer) == publication.factory(),
            "factory-shaped deployer"
        );

        VM.expectPartialRevert(
            HellboxPublication.PrizeWalletRequestAlreadyCreated.selector
        );
        publication.requestPrizeWalletIssuance(PRIZE_WALLET);
    }
    ...
    function testPrizeRequestRejectsProjectAndContractRecipients() public {
        ...
        for (uint256 i = 0; i < forbidden.length; ++i) {
            VM.expectPartialRevert(
                HellboxPublication.InvalidPrizeWalletRecipient.selector
            );
            publication.requestPrizeWalletIssuance(forbidden[i]);
        }

        NonReceiverPrizeTarget contractRecipient =
            new NonReceiverPrizeTarget();
    ...
    function testOrdinaryNonTailCannotPrecedePrizeBootstrap() public {
        DeterministicEvmnetVerifierMock verifier =
            new DeterministicEvmnetVerifierMock();
        (PrizeWalletPublicationHarness publication, ) =
            _deployReady(address(verifier), "prize-must-be-seventh");

        uint256 ordinaryTokenId = publication.issueNonTailPrimaryForTest(
            ORDINARY_PRIMARY,
            ORDINARY_PRIMARY,
            123
        );
        ...
        VM.expectPartialRevert(
            HellboxPublication.PrizeWalletIssuanceOrderInvariant.selector
        );
        publication.requestPrizeWalletIssuance(PRIZE_WALLET);
    }
    ...
    function testRoundReadinessAndWrongProofFailWithoutMutation() public {
        DeterministicEvmnetVerifierMock verifier =
            new DeterministicEvmnetVerifierMock();
        (PrizeWalletPublicationHarness publication, ) =
            _deployReady(address(verifier), "prize-proof-rollback");
        ...
        VM.expectPartialRevert(
            HellboxPublication.RandomnessRoundNotReady.selector
        );
        publication.fulfillNextRandomnessRequest(
            abi.encode(round, bytes32(uint256(1)))
        );
        ...
        VM.expectPartialRevert(
            DeterministicEvmnetVerifierMock
                .MockProofRoundMismatch
                .selector
        );
        publication.fulfillNextRandomnessRequest(
            abi.encode(round + 1, bytes32(uint256(1)))
        );
        _assertPendingUntouched(publication, requestId, round);
    }

    function testPermissionlessRealDrandProofMintsSeventhCopy() public {
        HellboxDrandEvmnetVerifier verifier =
            new HellboxDrandEvmnetVerifier();
        (PrizeWalletPublicationHarness publication, ) =
            _deployReady(address(verifier), "prize-real-drand-proof");

        VM.warp(
            OBSERVED_ROUND_TIMESTAMP -
                publication.RANDOMNESS_REQUEST_DELAY_SECONDS()
        );
        ...
        (uint256 fulfilledRequestId, uint256 tokenId) =
        ...
        require(
            publication.walletLifetimePrimaryUsed(PRIZE_WALLET) == 0,
            "prize consumed lifetime cap"
        );
        ...
        require(
            publication.nextPendingRandomnessRequestId() == 0,
            "queue not empty"
        );
        ...
        require(
            verifiedRandomness == OBSERVED_RANDOMNESS,
            "stored provider randomness"
        );
        ...
        VM.expectPartialRevert(
            HellboxPublication.RandomnessRequestQueueEmpty.selector
        );
        publication.fulfillNextRandomnessRequest(_observedProof());
    }
    ...
    function testCopy066CanBePrizeAndIsNotHardcodedTo007() public {
        DeterministicEvmnetVerifierMock verifier =
            new DeterministicEvmnetVerifierMock();
        (PrizeWalletPublicationHarness publication, ) =
            _deployReady(address(verifier), "prize-copy-066");
        ...
        for (uint256 nonce = 1; nonce <= 10_000; ++nonce) {
            ...
            uint256 entropyWord = publication.deriveRandomnessRequestEntropy(
                requestId,
                candidateRandomness
            );
            uint256 candidateIndex = publication.uniformIndexForTest(
                entropyWord,
                210
            );
        ...
        (, uint256 tokenId) = publication.fulfillNextRandomnessRequest(
            abi.encode(round, selectedRandomness)
        );
    ...
    function testRecipientMustRemainAnEoaThroughFulfillment() public {
        DeterministicEvmnetVerifierMock verifier =
            new DeterministicEvmnetVerifierMock();
        (PrizeWalletPublicationHarness publication, ) =
            _deployReady(address(verifier), "prize-eoa-revalidation");
    ...
    function testSameProviderWordIsDomainSeparatedAcrossPublications() public {
        ...
        (PrizeWalletPublicationHarness first, ) =
            _deployReady(address(verifier), "prize-domain-first");
        (PrizeWalletPublicationHarness second, ) =
            _deployReady(address(verifier), "prize-domain-second");

        (uint256 firstRequestId, uint64 firstRound) =
            first.requestPrizeWalletIssuance(PRIZE_WALLET);
        (uint256 secondRequestId, uint64 secondRound) =
            second.requestPrizeWalletIssuance(PRIZE_WALLET);

        require(firstRound == secondRound, "probe rounds differ");

        uint256 firstEntropy = first.deriveRandomnessRequestEntropy(
            firstRequestId,
            OBSERVED_RANDOMNESS
        );
        uint256 secondEntropy = second.deriveRandomnessRequestEntropy(
            secondRequestId,
            OBSERVED_RANDOMNESS
        );
