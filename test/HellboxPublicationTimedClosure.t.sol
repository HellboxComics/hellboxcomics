...
contract HellboxPublicationTimedClosureTest {
    ...
    function testTimedClosureDeadlineIsExactAndCannotQueueEarly() public {
        (PrizeWalletPublicationHarness publication, DeterministicEvmnetVerifierMock verifier) =
            _deployPrizeReady("timed-close-deadline");

        require(
            publication.nativeMintDeadline()
                == publication.frozenAtTimestamp() + 100 + publication.NATIVE_MINT_DURATION_SECONDS(),
            "paid-opening deadline derivation"
        );
        require(
            publication.nativeMintDeadline()
                != publication.frozenAtTimestamp() + publication.NATIVE_MINT_DURATION_SECONDS(),
            "deployment started clock"
        );
        require(publication.NATIVE_MINT_DURATION_SECONDS() == 5_724_366, "native duration");
        ...
        require(verifier.roundTimestamp(round) >= block.timestamp + 12, "future closure round");
    }
    ...
    function testTimedClosureRequestIsPermissionlessAndSingleShot() public {
        ...
        require(kind == HellboxPublication.RandomnessRequestKind.TIMED_CLOSURE, "closure kind");
        ...
        VM.expectPartialRevert(HellboxPublication.NativeClosureRequestAlreadyCreated.selector);
        publication.requestNativeClosure();
    }
    ...
    function testTimedClosureWrongProofRetriesSameFifoRequest() public {
        ...
        publication.fulfillNextRandomnessRequest(abi.encode(round + 1, bytes32(uint256(777))));

        require(publication.randomnessFulfillmentCount() == 1, "failed proof consumed fifo");
        require(!publication.tailAwarded(), "failed proof awarded tail");
        require(publication.candidatePoolRemaining() == 209, "failed proof candidate mutation");

        (uint256 fulfilledRequestId,) =
            publication.fulfillNextRandomnessRequest(abi.encode(round, bytes32(uint256(777))));
        require(fulfilledRequestId == requestId, "request rerolled");
        require(publication.randomnessFulfillmentCount() == 2, "retry not consumed");
    }
    ...
    function testTimedClosureFinalizesThreeAndExtinguishesRemainder() public {
        ...
        (uint256 fulfilledRequestId, uint256 requestTokenId) =
            publication.fulfillNextRandomnessRequest(abi.encode(round, bytes32(uint256(0xC105E))));
        ...
        require(publication.extinguishedUnmintedCount() == 206, "extinguished count");
        require(publication.closedAtTimestamp() == block.timestamp, "closed at");
        require(!publication.isCandidateCopyAvailable(66), "extinguished #066 still candidate");
        ...
        publication.issueNonTailPrimaryForTest(ORDINARY_PRIMARY, ORDINARY_PRIMARY, 123);
    }

    function testTimedClosureTrueMintOutFinalizesBeforeDeadlineWithoutExtinguishment() public {
        (PrizeWalletPublicationHarness publication, DeterministicEvmnetVerifierMock verifier) =
            _deployPrizeReady("timed-close-mintout");

        for (uint256 i = 0; i < 206; ++i) {
            address account = address(uint160(0x100000 + (i / 6) + 1));
            publication.issueNonTailPrimaryForTest(account, account, 0x1000 + i);
        }
        ...
        VM.warp(verifier.roundTimestamp(round));
        publication.fulfillNextRandomnessRequest(abi.encode(round, bytes32(uint256(0xF1A13))));
    ...
    function _deployPrizeReady(string memory publicationKey)
        internal
        returns (PrizeWalletPublicationHarness publication, DeterministicEvmnetVerifierMock verifier)
    {
        VM.warp(START_TIMESTAMP);
        verifier = new DeterministicEvmnetVerifierMock();

        PrizeWalletPublicationDeployer deployer;
        (publication, deployer) = _deployReady(address(verifier), publicationKey);

        require(publication.nativeMintDeadline() == 0, "deployment started clock");
        uint256 paidMintOpensAt = block.timestamp + 100;
        deployer.bindPrimarySaleClock(
            address(publication), paidMintOpensAt + publication.NATIVE_MINT_DURATION_SECONDS()
        );
        require(
            publication.nativeMintDeadline() == paidMintOpensAt + publication.NATIVE_MINT_DURATION_SECONDS(),
            "paid mint clock not bound"
        );

        (uint256 prizeRequestId, uint64 prizeRound) = publication.requestPrizeWalletIssuance(PRIZE_WALLET);
        require(prizeRequestId == 1, "prize request id");

        VM.warp(verifier.roundTimestamp(prizeRound));
        publication.fulfillNextRandomnessRequest(abi.encode(prizeRound, bytes32(uint256(0xB00757A9))));
