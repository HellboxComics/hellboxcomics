...
**Checkpoint:** production Prize Wallet publication integration is committed at `037a7236a60b194d177ffdc1241741525fc2ea3e`, post-integration documents at `78a4c804e01e1041b869fed3bff38ffdb2ef2060`, and the permanent actual-factory composition/adversarial regression alone at `151d545dc22065698324b7e894ded01db8b5603d`. The committed tree contains **125 Solidity tests**. Creator evidence is **5/5 focused actual-factory tests and 125/125 total tests passed, 0 failed, 0 skipped**. Production runtime sizes remain publication **19,330 bytes** / **5,246 bytes** headroom, factory **12,424 / 12,152**, BirthPolicy **5,259 / 19,317**, code store **62 / 24,514**, verifier **5,138 / 19,438**.
**Next frontier:** complete native timed closure through two narrow source boundaries—publication-only BirthPolicy inventory finalization, then publication FIFO `TIMED_CLOSURE`. The measured compiler alternatives save only 416–627 publication bytes, so the committed optimizer-200/non-IR profile remains frozen. Phase/eligibility/payment escrow and settlement move to a separate versioned non-upgradeable sale/mint-policy module; proceeds routing and renderer/art data remain separate boundaries. The publication keeps final queue, copy-selection, mint, lifetime-cap, supply and closure authority.
...
**New locked pricing correction:** Gate 4 V1 does not hard-code `$6.66` or PLS-only/release-wide pricing. The private Press may configure each collector phase as `FREE`, `FIXED_NATIVE` or one exact `FIXED_ERC20` route before `PUBLISH`; deployment freezes every phase's payment mode, asset and raw-unit amount.

**New locked on-chain artifact correction:** native Hellbox canonical ERC-721 metadata and evolving cover must be independently reconstructible from immutable chain code/data/state. Gate 4 must preserve and prove a lean immutable renderer interface/binding path; Gate 6 builds the full art-data compiler/renderer. An external URL or digest alone is not canonical art availability.
...
- production compiler profile is frozen and committed: optimizer enabled, `optimizer_runs = 200`, `via_ir = false`; a production-only comparison measured only 416 bytes saved at runs=1 and 627 bytes saved with via-IR, so neither alternative is adopted; any future change still requires a new measured/full-regression checkpoint;
...
- committed optimized Shanghai sizes after production Prize Wallet integration: `HellboxPublication` runtime **19,330 bytes** with **5,246 bytes** EIP-170 headroom and **29,345-byte** creation bytecode with **19,807 bytes before constructor arguments**; `HellboxPublicationFactory` runtime **12,424 bytes** with **12,152 bytes** headroom, **19,614-byte** creation size and **19,742-byte** initcode including constructor arguments with **29,410 bytes** EIP-3860 headroom; `HellboxDrandEvmnetVerifier` runtime **5,138 bytes** with **19,438 bytes** headroom;
...
- production randomness is frozen to drand `evmnet` through the factory-generation-bound stateless verifier; publication verifier binding, future-round request state, exact FIFO permissionless fulfillment, creator orchestration, real factory-registry reserve/complete calls and permanent actual-factory rollback/reentrancy coverage are committed;
...
- committed Gate 4 tree contains **125 Solidity tests** with creator evidence for **125 total passed, 0 failed, 0 skipped**:
  ...
  - 8 permanent Prize Wallet FIFO tests;
  - 15 permanent Prize Wallet registry/progressive tests;
  - 5 permanent actual-factory production-integration/adversarial tests;
...
- the seventh successful mint event for each standard native issue is a blind first-non-tail Prize Wallet issuance to the active campaign EOA, not guaranteed copy #007, with zero effective Archive reward weight while unclaimed;
- one fresh 12-word MetaMask-compatible campaign EOA persists across releases until the separate Byte-site escape room is solved; Harrow never receives withdrawal/claim/reroll power, the phrase, private key, final unlock key or answer sheet;
...
- a Harrow-controlled/arbitrary prize EOA, networked wallet generation or arbitrary prize-recipient setter;
...
## 0.2 Measured Gate 4 completion architecture — 2026-09-03

Production-only compiler measurements on the exact `151d545` tree:

| Profile | Publication runtime | Publication EIP-170 headroom | Decision |
|---|---:|---:|---|
| optimizer 200, `via_ir = false` | 19,330 B | 5,246 B | **authoritative** |
| optimizer 1, `via_ir = false` | 18,914 B | 5,662 B | reference only; 416 B saved |
| optimizer 200, `via_ir = true` | 18,703 B | 5,873 B | reference only; 627 B saved |

Compiler-profile changes do not solve the remaining architectural load. The proven optimizer-200/non-IR profile remains frozen. Gate 4 completion uses module boundaries instead of filling the publication to the hard limit.

### Responsibility boundary

`HellboxPublication` retains only responsibilities that must be authoritative at the token/supply layer:

- ERC-721 ownership/royalty interface and frozen release identity;
- FIFO request order and immutable verifier binding;
- lifetime primary usage;
- candidate pool, random copy selection and mint execution;
- primary-closure state, Final-3 issuance and permanent capacity accounting;
- immutable references and narrow calls to approved modules;
- narrow `tokenURI` forwarding to the frozen renderer.

`HellboxBirthPolicy` retains MARK/DEFECT assignment/inventory and receives only the publication-only finalization transition needed to extinguish unassignable timed-closure inventory truthfully.

A separate versioned, non-upgradeable sale/mint-policy module owns frozen phase schedule, eligibility, exact `FREE` / `FIXED_NATIVE` / `FIXED_ERC20` request-time payment escrow, phase allowances and completion/refund settlement. It may authorize and fund a request but may not mint, select a copy, choose entropy, bypass FIFO order, alter price after `PUBLISH`, alter lifetime usage, or rewrite publication supply. Exact ABI/deployment transport remains Gate 4 engineering proof, not a new collector decision.

Primary proceeds use a separate immutable routing boundary. Gate 4 proves safe handoff and does not embed future reward-token, buy/burn or changing split economics.

Canonical metadata/cover rendering and art bytes remain in separate immutable renderer/data-store contracts. Archive, rewards, Hellforge, ERC-6551 runtime behavior, Reader narrative state and Prize Capsule tooling remain outside the publication.

### Publication byte reserve

Gate 4 adopts a **2,048-byte publication runtime soft reserve** after remaining publication-facing changes. This is an engineering stop/review line, not an EVM rule. A measured candidate crossing it must be externalized, simplified without changing collector promises, or explicitly re-reviewed before commit. Hard EIP-170/EIP-3860 limits still apply independently.

### Remaining implementation order

1. BirthPolicy publication-only timed-closure inventory finalization + tests.
2. Publication permissionless FIFO `TIMED_CLOSURE`, deadline, unbiased Final 3 and permanent capacity closure + tests.
3. Separate sale/mint-policy module + tests.
4. Factory binding/registration for the exact module.
5. Narrow publication collector request/fulfillment bridge + tests.
6. Separate proving renderer/data store + narrow publication `tokenURI` binding.
7. PulseChain Testnet V4 SciVive and second-publication acceptance, hardening and Gate close.

No locked product rule is removed by this architecture update.

## 0.3 Solo-operator implementation constraint
...
## 6.1 Artifact Ownership Covenant / holder-license fields — REQUIRED FOR NATIVE ISSUES

Token ownership, artifact-state ownership and copyright are distinct. The Press must freeze a durable, plain-language rights record rather than relying on NFT folklore.

| Field | Type / examples | Home | Mutability | Commit | Public | Rule |
|---|---|---|---|---|---|---|
| `rights.copyrightOwner` | legal/pseudonymous rights-holder statement | `P D U I` | `FREEZE` | `ROOT/SUB` | yes | Must match actual rights. |
| `rights.holderLicenseId` | stable license/version ID | `P C D U I` | `FREEZE` | `ROOT/SUB` | yes | Mandatory for native issues. |
| `rights.holderLicenseDigest` | exact license-text digest | `P C I` | `FREEZE` | `ROOT/SUB` | yes/proof | Must resolve to durable exact text. |
| `rights.holderLicenseStorageMode` | immutable on-chain text and/or permanent content-addressed record + Continuity copy | `P C I` | `FREEZE` | `ROOT/SUB` | yes/proof | Mutable website terms cannot be the sole source. |
| `rights.transferRule` | follows-current-owner / other explicit rule | `P C U I` | `FREEZE` | `ROOT` | yes | No ambiguity after transfer. |
| `rights.personalUseScope` | display/avatar/backup/print/etc. | `P D U I` | `FREEZE` | `ROOT/SUB` | yes | State exactly what is granted. |
| `rights.commercialUseScope` | none / limited / defined grant | `P D U I` | `FREEZE` | `ROOT/SUB` | yes | Exact Native Issue #1 choice remains creator/legal approval. |
| `rights.trademarkCanonReservation` | explicit reservation | `P D U I` | `FREEZE` | `ROOT/SUB` | yes | Token ownership does not grant official-canon or trademark authority unless expressly stated. |
| `rights.canonicalArtStatement` | on-chain artifact ownership statement | `P D U I` | `FREEZE` | `ROOT` | yes | Must distinguish independently reconstructible art from copyright ownership. |

Minimum technical statement for a native issue:

> The holder owns and can transfer the ERC-721 and its unique on-chain artifact state, and can independently reconstruct the canonical cover from the source chain without permission from Hellbox or a marketplace.

Minimum legal-honesty statement:

> Copyright, trademarks and official-canon authority transfer only to the extent expressly granted in the frozen holder license.

The full license text may live as immutable on-chain data and/or a durably content-addressed object whose exact digest is committed, but the canonical metadata must expose the license ID/digest and a self-contained human-readable rights summary.
...
That table is historical evidence for the factory correction. The generated artifacts for the committed optimized 200-run implementation measure:

```text
HellboxPublication runtime                    = 19,330 bytes
HellboxPublication creation size              = 29,345 bytes
HellboxPublicationFactory runtime             = 12,424 bytes
HellboxPublicationFactory creation            = 19,614 bytes
factory initcode with constructor args        = 19,742 bytes
HellboxDrandEvmnetVerifier runtime             =  5,138 bytes
HellboxBirthPolicy runtime                    =  5,259 bytes
HellboxBirthPolicy creation size              = 12,188 bytes
CodeStore compiler runtime stub               =     62 bytes
CodeStore actual deployed runtime             = 12,189 bytes
CodeStore actual EIP-170 margin               = 12,387 bytes
HellboxBirthPolicyCodeStore creation          = 12,346 bytes
```

The earlier measured standard-native publication payload/headroom values were produced before the compiler-profile checkpoint and must be remeasured by the creator-side size/runway script after every constructor/configuration change; they are not silently carried forward as current optimized values.
...
derived deployed code length = 12,189 bytes
EIP-170 runtime limit          = 24,576 bytes
derived runtime margin         = 12,387 bytes
```
...
The active wiring topology therefore keeps BirthPolicy creation bytecode **out of publication initcode**. The publication `EXTCODECOPY`s the approved bytes from offset `1`, verifies the approved creation-code hash, appends canonical constructor arguments, and executes ordinary `CREATE` itself so `HellboxBirthPolicy.publication = msg.sender` binds to the actual publication.

This remains `FULL_DEPLOYMENT`; it is not a clone/proxy/upgrade model.

Do **not** resurrect either embedded publication creation bytecode in factory runtime or direct embedded BirthPolicy creation bytecode in publication initcode. Do **not** change the frozen optimizer-on/200-run/`via_ir=false` profile merely to hide a structural size regression.
...
| `supply.nonTailIssuanceCapacity` | derived non-tail primary issuance limit | builder | `P D U I` | `DERIVED` | `ROOT` | yes | Standard native: `216 - 6 - 3 = 207`. Exactly one of these slots is the locked Prize Wallet bootstrap, leaving at most 206 ordinary collector/partner issuances. It is **not** the initial randomness/odds denominator. |
...
## 11.2 Repeating promotional Prize Wallet — LOCKED

| Field | Standard native value | Home | Mutability | Commit | Public | Validation |
|---|---|---|---|---|---|---|
| `prizeWallet.enabled` | `true` | `C P D U I` | `FREEZE/CAPABILITY` | `ROOT/SUB` | yes | Required for standard native profile; SciVive exempt unless later explicitly configured. |
| `prizeWallet.issuanceOrder` | `SEVENTH_SUCCESSFUL_MINT / FIRST_NON_TAIL` | `C F P I` | template invariant | `ROOT` | yes | After #001–#006 and before any collector phase; not token ID #007. |
| `prizeWallet.drawPolicy` | `SAME_RANDOM_CANDIDATE_POOL` | `C F P U I` | template invariant | `ROOT` | yes | Draws from all 210 candidates through the same future-round FIFO drand path; no fixed MARK/DEFECT, reroll or special odds; #066 remains eligible. |
| `prizeWallet.recipientPolicy` | `ACTIVE_SELF_AUTHORIZED_CAMPAIGN_EOA_ONLY` | `F P D U I` | generation-bound operational state | `RECORD/EVENT` | transparent | Fresh no-code EOA self-signs EIP-712 activation; cannot be Harrow/factory/owner/pending-owner, previously used, or caller-selected by a publication. |
| `prizeWallet.recoveryPhrasePolicy` | `12_WORD_METAMASK_COMPATIBLE / NO_HARROW_SECRET` | `D P I` | campaign-build invariant | manifest/capsule digest | policy public; words secret | Complete ordered phrase is encrypted in the capsule and revealed locally only after the full high-entropy puzzle key is reconstructed. |
| `prizeWallet.puzzleSurface` | `PERSISTENT_BYTE_SITE_INTRO` | `P D U I` | campaign package freeze | package/manifest digest | yes | Separate from every comic/owner Reader; same package remains live as issue prizes accumulate. |
| `prizeWallet.campaignPersistence` | `UNTIL_DIRECT_WINNER_CONFIRMATION` | `F P D U` | ruled runtime state | event/registry | yes | No automatic expiry, per-issue rotation or unsolved reset. |
| `prizeWallet.publicationDepositPolicy` | `ONE_APPROVED_DEPOSIT_PER_OFFICIAL_PUBLICATION` | `F P U I` | generation-bound runtime | events/state | yes | Explicit owner approval, publication-only reserve/complete, pending deposits block claim. |
| `prizeWallet.claimAcknowledgement` | `RECOVERED_EOA_CALLS_FACTORY` | `F P U` | one-way runtime state | event/state | yes | Moves no assets; at least one completed deposit; zero pending deposits. |
| `prizeWallet.rewardEligibilityWhileUnclaimed` | `0` | `P U I` | external protocol invariant | `ROOT/POLICY` | yes | Prevents campaign-held comics from farming ordinary Archive rewards. |
| `prizeWallet.rotationPolicy` | `FRESH_EOA_AFTER_CLAIM_ONLY` | `P D U` | operational state | event/registry | yes | Old EOA remains winner-owned and can never be activated again. |
...
The publication must fail closed before collector mint opening if it cannot reserve the approved active generation, create/fulfill the request or complete its deposit.

Campaign flow:

```text
offline builder creates 12-word EOA + activation signature + encrypted capsule
→ factory activates wallet generation
→ issue A/B/C/... each contribute one approved seventh-mint comic
→ persistent Byte-site puzzle reconstructs unlock key
→ browser decrypts all 12 words locally
→ winner restores EOA and calls confirmPrizeWalletClaim()
→ fresh campaign may begin only after zero pending deposits
```

The campaign manifest binds the exact puzzle package, encrypted capsule, generation/address, terms and official-prize classification policy. Future approved issue contributions are appended by completed deposit events rather than requiring the puzzle package to change. Optional promotional transfers are official only through an append-only campaign disclosure; unsolicited transfers are ignored.

AI may design the public maze against dummy placeholders, but final secret-bearing clue assembly occurs locally. Harrow receives no phrase, private key, unlock key or answer sheet. Seed words are not revealed progressively. A public puzzle cannot cryptographically stop Harrow from solving the same clues, so fairness relies on no privileged answer material, public insider ineligibility and auditable operation rather than a false impossibility claim.

The old smart-contract-vault/recipient-bound commit-reveal design is **SUPERSEDED**. Reintroducing it requires a new explicit creator decision.
...
- keeps #066 eligible for the Prize Wallet, collector and timed-closure paths without making it targetable;
- makes the Prize Wallet the first non-tail FIFO request from all 210 candidates;
- does not reroll or assign the Prize Wallet from a special pool;
...
# 16. CANONICAL COVER & ON-CHAIN ART INPUTS

For every native Hellbox issue, `metadataPolicy.storageMode = FULLY_ONCHAIN` is required. The source art may begin as local SVG/PNG/PSD/etc., but publication output must contain every byte required to reconstruct the canonical collectible cover on-chain.

| Field | Type | Home | Mutability | Commit | Public | Validation |
|---|---|---|---|---|---|---|
| `contentPackage.baseCover.logicalId` | stable package ID | `P D I` | `FREEZE` | root/sub | yes | Required for collectible art. |
| `contentPackage.baseCover.sourceFileRef` | local/upload temp ref | `D` | `DRAFT` | `NO` | no | Never becomes a canonical production dependency. |
| `contentPackage.baseCover.digest` | source/canonical hash | `P I` | `FREEZE` | sub | proof | Must match normalized input/output rules. |
| `contentPackage.baseCover.mediaType` | SVG/vector/raster input class | `P I` | `FREEZE` | root | proof | Must compile to supported on-chain representation. |
| `contentPackage.baseCover.width/height` | derived dimensions | `P I` | `FREEZE` | root | preview | Validate. |
| `contentPackage.canonicalCompletenessDigest` | statement/hash proving the on-chain rendition is the complete collectible cover rather than a placeholder/proxy | `P C I` | `FREEZE` | `ROOT/SUB` | yes/proof | Must match the final freeze preview. |
| `contentPackage.onchainArtEncodingId` | approved encoding/compression profile | `F P C I` | `FREEZE` | `ROOT/SUB` | proof | Exact decoder/renderer compatibility. |
| `contentPackage.onchainArtDataRoot` | root of immutable art-data chunks | `P C I` | `FREEZE` | `ROOT/SUB/DIRECT` | yes/proof | Required for native issues. |
| `contentPackage.onchainArtChunkManifestDigest` | ordered chunk/address/data manifest | `P C I` | `FREEZE` | `SUB` | proof | Every required chunk accounted for. |
...
| `contentPackage.sharedRenderAssets[]` | palettes/fonts-as-outlines/textures/masks/etc. | `P D I` | `FREEZE` | sub-manifest | indirect/proof | Every dependency needed for zero-host reproduction must become immutable on-chain data or deterministic code. |
| `contentPackage.artRightsRefs[]` | license/source references | `P D I` | `FREEZE` | root/sub | package/metadata | Required where applicable. |

Rules:

- a mutable URL, IPFS CID, Arweave transaction ID or digest alone does not satisfy `FULLY_ONCHAIN`;
- ERC-721 JSON uses the standard `image` field for the self-contained canonical cover; `image_data` may be duplicated only as optional compatibility data and may not replace `image` as the sole canonical surface;
- IPFS/Arweave/R2 copies are allowed as mirrors/derivatives, never the only canonical art bytes;
- font dependencies must be legally usable and stored/converted so reconstruction does not depend on a user's installed font;
- high-resolution print/web derivatives may remain off-chain if the canonical on-chain cover and its relationship to derivatives are explicit; they may improve resolution/print production but cannot contain the only complete artwork;
- the on-chain rendition cannot be a thumbnail, placeholder, blurred preview, trait-only proxy or hash pointer;
- vector/optimized SVG is the standard low-cost path for a self-funded Press;
- raster plates are allowed only when complete compressed raw bytes are chunked into immutable on-chain stores and deterministically Base64-embedded by the renderer with no external `<image href>` dependency;
- the private Press must preview exact data-store count, payload bytes, expected deployment cost and maximum `tokenURI` return size before freeze;
- a large raster release that fails the cost/size/compatibility budget must be simplified/vectorized or deferred rather than mislabeled fully on-chain;
- art packing must pass contract-size, deployment-cost, RPC-return-size and marketplace-rendering tests;
- default production is deterministic/reproducible compositing from Harrow-approved source art, not runtime AI generation.

SciVive and explicitly classified archival/proving releases may use a narrower committed-content mode. That exception cannot silently become the Native Issue #1 standard.

---

# 17. IMMUTABLE METADATA RENDERER / COMPOSITOR

The publication contract must remain a lean state/provenance kernel. The full compositor and art bytes belong in versioned immutable sidecars/data stores.

Preferred topology:

```text
HellboxPublication.tokenURI(tokenId)
    → immutable renderer reference
    → renderer runtime-code-hash/version check
    → immutable publication art-data store(s)
    → publication/BirthPolicy/later-protocol on-chain state
    → self-contained JSON data URI
    → standard `image` field containing self-contained canonical cover data URI
```

No `delegatecall` is required or permitted for rendering. Renderer/data-store runtimes also expose no reachable `SELFDESTRUCT`, proxy/upgrade path or code-deletion/replacement mechanism; an immutable address is not enough if its code can disappear. The write-once-contract-code/`EXTCODECOPY` pattern may be reused, but a third-party SSTORE2 implementation is not approved merely by name. Gate 6 should use a minimal reviewed Hellbox inert data-store primitive with a non-executable prefix, bounded reads, exact byte length/hash verification and no caller-selected executable payload.

Existing `HELLBOX_ABI_V1` anchors already include `ReleaseConfig.dynamicMetadataEnabled` plus `CommitmentSet.rendererRulesDigest`, `metadataPolicyDigest`, `publicationManifestDigest`, `packageDigest`, `capabilityPolicyDigest`, `protocolCompatibilityDigest` and `eventPolicyDigest`. They permit a canonical deployment-time renderer/art-data/rights enforcement context to be transported and verified under the already-approved preimage doctrine; they do not make art available by themselves. No field is reordered or reinterpreted merely to add the renderer binding.
...
| `renderer.interfaceVersion` | metadata/render interface generation | `F C X P I` | `FREEZE` | `ROOT` | proof | Gate 4 kernel must establish/prove this compatibility point. |
| `renderer.rendererAddress` | exact non-upgradeable renderer | `C F X P I` | `FREEZE` | `ROOT/DIRECT/context` | yes/proof | Immutable in publication; no setter. |
| `renderer.runtimeCodeHash` | exact deployed renderer runtime hash | `F C P I` | `FREEZE` | `ROOT/DIRECT/context` | yes/proof | Must match approved generation. |
| `renderer.artDataStoreAddresses[]` | immutable code/data stores | `C F P I` | `FREEZE` | `ROOT/SUB/context` | yes/proof | Ordered and bound to manifest/root. |
| `renderer.artDataStoreCodeHashes[]` | runtime/data hashes | `F C P I` | `FREEZE` | `ROOT/SUB/context` | yes/proof | Detect missing/replaced data. |
| `renderer.rulesDigest` | committed compositor rules | `C P I` | `FREEZE` | `SUB/DIRECT` | proof | Must match package. |
| `renderer.renderEnvironmentVersion` | compiler/reproducibility version | `P I` | `FREEZE` | `ROOT` | proof | Prevents undocumented output drift. |
...
| `renderer.variantMode` | on-chain dynamic composite / on-chain prepacked variants | `P C I` | `FREEZE` | `ROOT` | proof | No off-chain canonical renderer. |
| `renderer.optionalContextFallbackPolicy` | frozen bounded adapter + deterministic base-cover fallback | `C F X P I` | `FREEZE` | `ROOT/SUB` | yes/proof | A missing/reverting/malformed optional source cannot brick metadata or erase the base cover. |
| `renderer.reproducibilityReportDigest` | build/clean-room proof | `P D I` | `FREEZE` | `ROOT/SUB` | proof | Mandatory before Native Issue #1. |
...
Representative outputs are previewed before `PUBLISH`; Harrow does not manually author every token combination. Optional contextual/external protocol state may add only frozen, bounded overlays with a deterministic on-chain fallback; it may not be the only source of the base cover or unanchored permanent artifact truth. A broken old renderer is not upgraded in place: a new approved renderer/publication generation is used for future issues.

Default native painter order, with unused layers omitted and exact per-issue order frozen:

```text
1. complete shared PLATE
2. immutable PRESS MARK
3. immutable PRESS DEFECT
4. finite permanent artifact HISTORY / experience layers
5. bounded reversible current CONTEXT / status layers
6. ARCHIVE protective plastic/slab/label last
```

Birth layers never reroll. Already-earned permanent marks may remain visible beneath Archive plastic, but no new handling-derived state accumulates while archived. Defect effects such as REDACTED/BLED OUT/CORRUPTED PLATE may be masks/vector effects and OFF REGISTER may be a deterministic plate/layer transform; these are renderer-rule examples, not generic publication hard-coding.

Canonical SVG accepts only frozen/sanitized publication text, enum/numeric values and deterministic state. Scripts, event handlers, external stylesheets/fonts/media, remote URLs, `javascript:` surfaces, wallet-supplied markup and unbounded user-controlled strings are prohibited.

Time-derived changes are finite frozen milestones, not continuous invisible `block.timestamp` drift. Bare `block.timestamp - mintedAt` is insufficient for handling/raw-age wear because Archive must pause the covered accumulation. A marketplace-visible threshold requires a permissionless checkpoint/finalization path that stores the reached milestone and emits ERC-4906-compatible signaling. Archive accounting must freeze raw-age/handling accumulation rather than allow hidden damage to continue beneath the slab. If transfer-derived state is enabled, every ownership path must flow through the pinned ERC-721 implementation's canonical ownership-update hook—currently `_update` in the pinned OpenZeppelin V5 line—with explicit mint/burn/archive behavior; overriding only `transferFrom`/`safeTransferFrom` is prohibited. The exact compact state module and hooks belong to Gates 6–7; Gate 4 preserves the immutable renderer/state interface without adding experience mutators to the publication kernel.
...
# 19. V1 CONFIGURABLE PHASE-PRICING MODEL

Gate 4 V1 does **not** hard-code `$6.66`, PLS-only payment, one release-wide price or one universal whitelist/early/public price. The private Press may edit draft pricing freely before `PUBLISH`; the resolved phase policies then freeze at deployment.
...
On PulseChain, `FIXED_NATIVE` is native PLS. PLS is the standard/default paid preset, not the only selectable asset.

V1 simplicity boundary:

- each collector phase references exactly one frozen pricing policy;
- a paid phase references exactly one frozen payment route;
- phases may share policies/routes or intentionally differ;
- allowlist/whitelist, reserved/partner, early-Press and public phases may be free or have different exact prices/assets;
- one phase does not offer simultaneous interchangeable token choices in V1;
- no post-deployment pricing/asset setter exists, including before the first collector mint;
- no USD-target, oracle-adjusted or live-conversion pricing exists in V1.

`$6.66` may be used as optional Harrow-facing campaign copy, a private-Press preset or an off-chain drafting target. It is never a protocol constant, mandatory price, floor, ceiling or guaranteed fiat value.

The richer model remains inside the existing `pricingPoliciesDigest`, `paymentRoutesDigest` and `mintPhasesDigest` commitments; `HELLBOX_ABI_V1` field order does not change.

## 19.1 V1 pricing policy fields

| Field | Type | Home | Mutability | Commit | Public | Validation |
|---|---|---|---|---|---|---|
| `pricingPolicies[].pricingPolicyId` | stable release-local ID | `C P D U I` | `FREEZE` | `ROOT` | yes | Unique; may be referenced by one or more phases. |
| `pricingPolicies[].mode` | `FREE`, `FIXED_NATIVE` or `FIXED_ERC20` | `C P D U I` | `FREEZE` | `ROOT/DIRECT` | yes | No other V1 mode accepted. |
| `pricingPolicies[].rawUnitAmount` | uint256 smallest-unit amount | `C P D U I` | `FREEZE` | `ROOT/DIRECT` | yes | Zero only for `FREE`; nonzero for paid modes. |
| `pricingPolicies[].displayAmount` | human-facing formatted amount | `P D U I` | `FREEZE` | `ROOT` | yes | Must derive from raw amount + frozen/displayed asset metadata; never settlement authority. |
| `pricingPolicies[].currencyDisplayRef` | symbol/name/decimals snapshot | `P D U I` | `FREEZE` | `ROOT/SUB` | yes | Display only; cannot override chain/address/raw amount. |
| `pricingPolicies[].exactPaymentPolicy` | hard `EXACT_OR_REVERT` for V1 | `C F P U I` | `FREEZE` | `ROOT` | yes/proof | Native under/overpayment and ERC-20 short receipt revert atomically. |
| `pricingPolicies[].enabled` | bool | `C P I` | `FREEZE` | `ROOT` | yes | Omitted/disabled policies cannot be activated later. |

### `FREE`

- `rawUnitAmount = 0`;
- payment route is absent/`NONE`;
- `msg.value = 0`;
- no token transfer.

### `FIXED_NATIVE`

- exact nonzero native-unit amount frozen per referenced phase;
- on PulseChain, native settlement asset is PLS and asset address uses the zero/native sentinel;
- exact `msg.value` required;
- no refund tolerance or live fiat conversion.

### `FIXED_ERC20`

- exact nonzero amount in one frozen same-chain token's smallest units;
- one nonzero token-contract address with deployed code;
- zero native `msg.value`;
- pinned safe ERC-20 transfer handling and reentrancy-safe mint ordering;
- token-transfer failure or short balance receipt reverts atomically;
- fee-on-transfer, rebasing, callback-dependent or materially nonstandard assets are rejected by V1 validation/enforcement unless a later reviewed version explicitly supports them;
- symbol/name/decimals are display hints, while chain ID + token address + raw amount define settlement.

---

# 20. V1 ACCEPTED PAYMENT ROUTES

V1 supports a simple deterministic route per phase rather than a choose-your-token checkout.

| Field | Type | Home | Mutability | Commit | Public | Validation |
|---|---|---|---|---|---|---|
| `paymentRoutes[].routeId` | stable release-local route ID | `C P D U I` | `FREEZE` | `ROOT` | yes | Unique. |
| `paymentRoutes[].pricingPolicyId` | reference | `C P I` | `FREEZE` | `ROOT` | yes | Must reference an enabled compatible policy. |
| `paymentRoutes[].assetKind` | `NONE`, `NATIVE` or `ERC20` | `C P U I` | `FREEZE` | `ROOT/DIRECT` | yes | Must match pricing mode exactly. |
| `paymentRoutes[].assetAddress` | zero/native sentinel or ERC-20 contract | `C P D U I` | `FREEZE` | `ROOT/DIRECT` | yes/proof | Zero for `NONE/NATIVE`; nonzero deployed same-chain code for `ERC20`. |
| `paymentRoutes[].assetDisplaySymbol` | human display symbol | `P D U I` | `FREEZE` | `ROOT` | yes | Display only; contract address/chain are authoritative. |
| `paymentRoutes[].assetDecimalsSnapshot` | display integer | `P D U I` | `FREEZE` | `ROOT/SUB` | yes | Display formatting only; raw amount remains authoritative. |
| `paymentRoutes[].enabled` | bool | `C P D U I` | `FREEZE` | `ROOT` | yes | Disabled routes cannot be activated after publication. |
| `paymentRoutes[].settlementRouterRef` | durable Hellbox primary-proceeds routing endpoint for paid routes | `C X P I` | `FREEZE` | `ROOT/DIRECT` | proof | May route downstream operationally; cannot alter frozen asset/amount. |
| `paymentRoutes[].tokenCompatibilityPolicy` | exact-transfer V1 policy ID | `C F P I` | `FREEZE` | `ROOT` | yes/proof | Reject incompatible fee/rebase/callback behavior. |

A `FREE` phase has no paid route. Each paid phase selects exactly one route before `PUBLISH`. Different phases may select different routes/assets, but the route referenced by a phase cannot change after deployment.

There is no global post-publication token allowlist capable of replacing the frozen asset. Harrow's prelaunch choice is open to any standards-compatible token that passes the Press validation matrix; incompatible/malicious token behavior fails closed.

No PLS/USD or token/USD oracle is required merely to settle an exact amount. Back-office accounting may separately record transaction-time fiat fair-market values without becoming pricing authority.

The settlement routing endpoint may point to a stable Hellbox operational router whose downstream recipients, split percentages and future reward-token actions are mutable under the separate gated routing-control system described in §25. That operational mutability does **not** permit a publication phase's payment mode, asset or price to change.
...
The Prize Wallet issuance is also not a collector phase and does not consume a human wallet's lifetime cap. No reserved/free/allowlist/early/public phase may open until the one-time first-non-tail Prize Wallet issuance has completed successfully.
...
| `mintPhases[].pricingPolicyId` | reference to one enabled phase pricing policy | `C P D U I` | `FREEZE` | `ROOT` | yes | Different phases may intentionally reference different frozen free/native/ERC-20 policies. |
| `mintPhases[].paymentRouteId` | one route reference or none | `C P D U I` | `FREEZE` | `ROOT` | yes | None for `FREE`; exactly one compatible frozen route for a paid V1 phase. |
...
For Gate 4 V1, each phase freezes its eligibility, allocation, timing and pricing reference at `PUBLISH`. Whitelist/allowlist, early and public phases may intentionally differ, but no phase can be repriced or pointed at another asset after deployment. SciVive remains `FREE`.
...
| `publicPressRuntime.prizeWallet` | active EOA generation/address + claimed/pending/completed approved-publication deposit state + campaign package digest | `C/P U A` | `DERIVED/RULED` | `NO` | yes |
...
The exact router contract, routing authority and private/gated operational control interface remain technical work. That authority is an external economic-routing control surface, not generic ownership of collector NFTs and not permission to rewrite any phase's frozen payment mode/asset/raw-unit price, royalty BPS, supply, rarity, ownership or native deadline.

---

# 26. FULLY ON-CHAIN DYNAMIC METADATA POLICY

Dynamic metadata is mandatory architecture for native Hellbox artifacts and permitted for SciVive's narrower dynamic-cover/seal path. Native canonical metadata/cover must be self-contained and reconstructible without an HTTP/API/gateway dependency.
...
| `metadataPolicy.storageMode` | `FULLY_ONCHAIN` / classified proving exception | `C F P D U I` | `FREEZE` | `ROOT/DIRECT` | yes | Native standard: `FULLY_ONCHAIN`. |
| `metadataPolicy.selfContainedTokenUri` | bool | `C F P U I` | `FREEZE` | `ROOT/template` | yes/proof | Native standard: true. |
| `metadataPolicy.externalCanonicalDependencyAllowed` | bool | `C F P U I` | `FREEZE` | `ROOT` | yes/proof | Native standard: false. Mirrors are non-authoritative. |
| `metadataPolicy.artifactRightsDigest` | exact holder-rights/license digest | `C P I` | `FREEZE` | `ROOT/SUB/DIRECT` | yes/proof | Mandatory for native issues. |
...
Native dynamic metadata uses ERC-4906-compatible `MetadataUpdate` / `BatchMetadataUpdate` signaling when canonical JSON meaningfully changes. The event tells indexers to re-read `tokenURI`; it is not authority to fetch a private replacement image or mutate frozen renderer/art data.
...
Current proof status: `testJavascriptAndSolidityGoldenVectorMatch()` remains part of the **125-test committed tree**. Creator evidence for the exact committed bytes records **125/125 passed, 0 failed, 0 skipped**, including the permanent eight-test FIFO suite, **15/15 registry tests**, and **5/5 focused actual-factory integration tests**. The issuance fuzz boundary remains 256 runs.
...
Future pricing/payment/royalty/treasury enforcement preimages must preserve the same commitment boundary: immutable per-phase payment mode/asset/raw-unit price and phase-route assignment plus per-issue royalty BPS and routing endpoint/policy boundary may be committed, while intentionally mutable downstream recipient wallets, split percentages and reward-token strategy must not be accidentally reclassified as immutable collector promises.
...
| `privateDraft.prizeWalletGenerationRef` | `D P` | `DRAFT` | `NO` | no — proposed campaign generation/address + factory activation parameters. |
| `privateDraft.prizeCapsuleRef` | `D P` | `DRAFT` | `NO` | no — encrypted capsule/package digest only; never phrase/private key/final unlock key. |
...
- each phase pricing mode (`FREE`, `FIXED_NATIVE` or `FIXED_ERC20`);
- each phase accepted payment asset/contract;
- exact frozen raw-unit amount for each paid phase;
...
## 43.7 Renderer / fully on-chain canonical art

Blocking for native issues:
- metadata storage mode is `FULLY_ONCHAIN`;
- exact renderer ID/version/interface/address/runtime code hash match the approved generation;
- all art-data store addresses/hashes match the ordered on-chain chunk manifest/root;
- a clean-room RPC-only reconstruction returns self-contained token metadata and the complete approved canonical cover—not a thumbnail/placeholder/proxy;
- JSON contains a standard self-contained `image` field; optional `image_data` cannot be the sole image surface;
- reconstruction makes zero HTTP/DNS/CDN/IPFS-gateway/marketplace calls;
- no mutable base URI, renderer setter, art-data replacement, proxy upgrade, reachable `SELFDESTRUCT`/code deletion or admin image override exists;
- every dynamic visual input comes from explicit authoritative on-chain state under frozen rules;
- painter order is deterministic and Archive plastic/slab is last when active;
- no scripts/remote refs/event handlers/unbounded user markup exist in SVG/metadata;
- time-derived metadata changes use finite checkpointed milestones and ERC-4906-compatible events; Archive freezes relevant accumulation;
- transfer-derived state is tested through the pinned ERC-721 canonical ownership-update hook across mint, transfer, safe transfer and burn paths without bypass/double-counting;
- optional contextual/external sources fail safely to a deterministic base cover and cannot become unanchored permanent state;
- representative maximum-size metadata outputs remain within tested RPC/marketplace/browser limits;
- vector and permitted raster paths both prove exact on-chain byte completeness, store hashes and deployment-cost budget;
- rights/license ID, digest and human-readable summary are present and consistent with credits/provenance.
...
- every enabled collector phase references exactly one compatible pricing policy;
- mode is only `FREE`, `FIXED_NATIVE` or `FIXED_ERC20`;
- SciVive/proving free release has zero price and no paid route;
- native PLS is the standard paid Press preset, while a phase may select one validated same-chain ERC-20 before `PUBLISH`;
- `$6.66` is never a protocol constant or mandatory price;
- every paid policy freezes one nonzero exact raw-unit amount;
- ERC-20 routes require nonzero deployed code, exact safe transfer/balance behavior and zero native value;
- no USD target, price-conversion adapter, quote freshness, simultaneous multi-asset equivalence or tolerance logic exists;
- payment route is none for `FREE` or exactly one compatible native/ERC-20 route for a paid phase;
- no phase price/asset/reference can change after `PUBLISH`;
- exact-payment/overpayment/refund/revert behavior is tested.

## 43.9 Prize Wallet + mint phases

Prize Wallet blocking checks:

- active campaign generation exists, is unclaimed and passes wallet/manifest integrity checks;
- campaign address is a fresh self-authorized no-code EOA, not Harrow/factory/owner/pending-owner and not previously used;
- EIP-712 activation signature binds this factory, chain/domain, next generation, wallet, manifest digest and deadline;
- publication was physically created/registered by the same factory and explicitly approved for the active generation;
- publication has no prior Prize Wallet reserve/complete state;
- creator #001–#006 are complete before reserve/request;
- reserve precedes request; successful random issuance precedes completion;
- the prize request is the first non-tail FIFO request from all 210 candidates;
- no full phrase/private key/unlock key/answer sheet appears in package/config/source maps/logs/Harrow-facing output;
- campaign puzzle is the separate persistent Byte-site introduction, not a per-comic Reader room;
- no collector phase opens before this publication's deposit completes;
- pending deposits block claim and campaign rotation;
- used wallet and campaign manifest cannot be reused;
- completed publication-deposit events, not arbitrary transfers, define official comic additions;
- campaign rotation is impossible before direct winner-wallet confirmation.

Phase checks:

- phase IDs/order unique and valid;
- all eligibility roots/lists are complete;
- wallet lifetime caps cannot be bypassed through phases;
- every phase has exactly one frozen payment route;
- native deadline/go-live alignment valid;
- no post-freeze repricing route exists.
...
- exact per-phase payment mode/asset/raw-unit price plus royalty BPS/native deadline shown where applicable;
...
## 44.6 Renderer / ownership preview

Show and prove:
- exact renderer generation/address/runtime code hash;
- immutable art-data stores and aggregate on-chain data root;
- decoded self-contained `tokenURI` JSON;
- canonical cover rendered from a local RPC-only clean-room script with network access disabled except RPC;
- representative base, rare MARK/DEFECT, archived, unsealed and evolved/contextual states where enabled;
- metadata size and marketplace fallback behavior;
- copyright owner, holder-license ID/digest, rights summary, transfer rule and trademark/canon reservation;
- explicit statement that owning the token/artifact does not silently transfer copyright beyond that license.
...
- each phase mode: `FREE`, `FIXED_NATIVE` or `FIXED_ERC20`;
- accepted asset: none, native PLS or one frozen standards-compatible same-chain ERC-20/PRC-20;
- exact frozen raw-unit amount for every paid phase;
- frozen royalty BPS;
...
## 44.8 Prize Wallet + phase simulation

Show/simulate:

- proposed/active campaign generation, public EOA and manifest/package digest;
- EIP-712 activation recovery to the exact EOA without revealing the phrase;
- same wallet receiving one random seventh-mint comic from multiple distinct official publications;
- `210/207 → 209/206` after each publication's own prize fulfillment;
- pending/completed deposit event accounting and claim blocking;
- generation-bound publication approval and no approval leakage after rotation;
- local capsule decryption test against a worthless dummy phrase/package;
- direct recovered-wallet claim acknowledgment and fresh next-generation activation;
- public jackpot display ignoring spam transfers and using completed deposits/official disclosures;
- every phase's eligibility/payment/deadline behavior after prize completion.

Do not use the live phrase or final unlock key in Harrow-facing preview. Production campaign validation operates on public artifacts and sealed local builder evidence.
...
promotional Prize Wallet:
  ...
  recipient: approved active campaign EOA only
...
primary pricing preset:
  public phase mode: FIXED_NATIVE
  payment asset: native PLS (default preset, editable before PUBLISH)
  exact raw-unit amount: issue/phase-specific, frozen at PUBLISH
  allowlist/early/reserve phases: independently configurable FREE/FIXED_NATIVE/FIXED_ERC20
  ERC-20 route: one validated same-chain token per paid phase when selected
...
1. **Prize Wallet incident/liveness operations**
   - fail-closed behavior and operator alerting if an already-reserved request cannot be fulfilled for an extended period;
   - no alternate entropy, reroll, redirect or public phase while a publication deposit remains pending;
   - optional append-only disclosure mechanics for non-publication promotional assets remain separate tooling.

2. **Native timed closure**
   - exact closure request trigger and FIFO ordering relative to collector/prize requests;
   - exact liveness/failure behavior when a drand proof is unavailable or invalid, with no alternate entropy;
   - unbiased Final-3 selection at expiry when more than three candidates remain;
   - permanent extinguishment and no Harrow/manual grail-selection path.

3. **Metadata renderer transport / on-chain art binding**
   - exact Gate 4 interface, immutable renderer/data-store binding context and small proving renderer;
   - self-contained native `tokenURI` path with no mutable external canonical dependency;
   - factory-generation approval versus publication-specific data-store topology, selected only after compile/size/security evidence;
   - Gate 6 implements the full deterministic package/art-data compiler and production renderer.

4. **Separate sale/mint-policy + routing enforcement**
   - exact versioned non-upgradeable module ABI and factory/publication binding;
   - request-time native/ERC-20 escrow, exact balance-delta and nonstandard-token rejection behavior;
   - phase allowance, completion, refund/liveness and anti-queue-griefing behavior;
   - stable proceeds-router interface and rotation-safe authority boundary;
   - publication glue must preserve the 2,048-byte soft reserve under the frozen optimizer-on/200-run/non-IR profile.

5. **Offline Prize Capsule Builder**
   - exact authenticated-encryption/KDF choices and browser compatibility;
   - secure temporary-memory/file deletion and crash recovery before activation;
   - reproducible package/build evidence and independent review;
   - append-only disclosure mechanism for optional non-publication promotional assets.

The compiler profile, drand provider, FIFO request/fulfillment model, 12-word EOA custody model, separate Byte-site puzzle, cross-release persistence and direct winner-wallet claim are no longer open.
...
- only a fresh no-code EOA self-authorized by EIP-712 may become active;
- no arbitrary/Harrow-controlled recipient, phrase, private key, unlock key or answer sheet in source/config/logs/AI;
- same future-round FIFO draw and no reroll/special trait pool;
- persistent Byte-site puzzle is separate from comics and uses a high-entropy capsule key, not gradual seed-word clues;
- wallet remains active across releases until direct winner confirmation; no auto-expiry/reset;
- only factory-created and generation-approved publications reserve/complete once;
- pending deposits block claim/rotation and require monitoring plus a reviewed liveness policy;
- completed approved-publication deposits and explicit append-only disclosures define official prizes; spam transfers do not;
- zero Archive reward weight while unclaimed;
- post-restore UX confirms claim before sweep and includes a small disclosed PLS gas stipend;
- Harrow/household/operators are publicly ineligible, but public clues remain publicly solvable and no false cryptographic exclusion is claimed.
...
This blueprint remains approved. Gate 4 may continue only along the dependency-ordered publication/factory/issuance path. It must not leap from the current substrate directly to a public mint.

## Committed/pushed proof boundary

Latest source/test implementation baseline before this documentation-only synchronization: `037a7236a60b194d177ffdc1241741525fc2ea3e`.

- exact Solidity `0.8.36`, Shanghai, optimizer-on/200-runs/`via_ir=false` profile;
- pinned OpenZeppelin `v5.1.0`;
- constructor-frozen full-deployment `HellboxPublication V1` and size-safe factory provenance;
- frozen `HELLBOX_ABI_V1` + JavaScript/Solidity golden vector;
- enforcement-preimage anchors;
- immutable BirthPolicy code store, publication-owned deployment and companion provenance;
- permanent per-token MARK/DEFECT assignment/inventory enforcement;
- frozen drand `evmnet` verifier and immutable factory-generation/publication binding;
- future-round append-only request state and exact permissionless FIFO fulfillment;
- production creator #001–#006 initialization, active-campaign reservation, one-time Prize Wallet first-non-tail request and atomic deposit completion with EOA validation and `210/207 → 209/206` accounting;
...
- **125 committed Solidity tests**, including the eight-test Prize Wallet FIFO probe, 15-test registry/progressive probe and five-test actual-factory production-integration probe;
- exact creator run: **5 focused actual-factory / 125 committed total passed, 0 failed, 0 skipped**.

Committed Prize Wallet identities:
...
The registry test still uses publication actors because it isolates factory behavior. Production `HellboxPublication` calls the reserve/request/fulfill/complete path at `037a723`, and commit `151d545` permanently proves that path against the actual factory, including rollback/retry, receiver-reentrancy and campaign-wallet code-drift boundaries.
...
- expose the public collector mint only after all prior fail-closed tests pass;
- bind/prove the lean immutable renderer/data/`tokenURI` path;
- deploy SciVive and a second dummy publication on PulseChain Testnet V4;
- prove a real mint through Gate 3 ownership/library/Reader recognition.

## Explicitly outside Gate 4

- final Press V2 UX and production Prize Capsule Builder — Gate 5/6 tooling;
- full interactive narrative/on-chain-art compiler and Reader runtime — Gate 6;
- Archive rewards/Hellforge/ERC-6551 protocols — Gate 7;
- Hellion system — Gate 8;
- final audit/content/localization/performance/operations/continuity/legal hardening — Gate 9;
- mainnet Native Issue #1 — Gate 10.

Do not add narrative graph state, puzzle timers, saved-run state, experience-mark mutation, Archive weight tables, reward emissions or burn-reward formulas to `HellboxPublication` or `HellboxBirthPolicy`.

# 49A. INTERNAL ENGINEERING CHECKPOINT — PRODUCTION PRIZE WALLET INTEGRATION COMMITTED

The publication randomness verifier binding, FIFO request/fulfillment substrate, factory campaign registry, permanent 15-test progressive registry probe, real production reserve/request/complete orchestration and permanent five-test actual-factory composition/adversarial proof are committed. No collector-facing mint endpoint exists. The immediate work is the two-file timed-closure boundary, followed by the separate sale/mint-policy module and narrow publication bridge.
...
- §§19–20 configurable per-phase V1 FREE/native/ERC-20 pricing/payment;
- §21 wallet/transaction limits;
...
The release freezes `maxSupply = 216`, every phase pricing mode/asset/raw-unit amount, royalty BPS, go-live time and native deadline.
...
Normal issuance remains internal-only at this checkpoint. It does **not** yet enforce public phases, deadline, native/ERC-20 phase payment or a production entropy source.
...
Under the recovered optimized 200-run artifacts, code-store creation is **12,346 bytes**. `forge build --sizes` still reports the compiler's nominal runtime stub as **62 bytes**, but the constructor returns `STOP || HellboxBirthPolicy.creationCode`; the actual deployed inert runtime is therefore **12,189 bytes** (one byte plus the **12,188-byte** BirthPolicy creation code), leaving **12,387 bytes** of EIP-170 headroom. Both nominal and actual values must remain in the size script because the compiler table alone does not express the constructor-returned runtime payload.
...
Current optimized-artifact proof:

- publication runtime **19,330 bytes** / EIP-170 margin **5,246 bytes**; creation bytecode **29,345 bytes** / EIP-3860 room before constructor arguments **19,807 bytes**;
- factory runtime **12,424 bytes** / EIP-170 margin **12,152 bytes**; creation size **19,614 bytes**; initcode with constructor arguments **19,742 bytes** / EIP-3860 margin **29,410 bytes**;
- drand verifier runtime **5,138 bytes** / EIP-170 margin **19,438 bytes**;
- BirthPolicy runtime **5,259 bytes** / EIP-170 margin **19,317 bytes**; creation size **12,188 bytes**;
- code-store compiler nominal runtime **62 bytes**; actual deployed inert runtime **12,189 bytes** / actual EIP-170 margin **12,387 bytes**; creation size **12,346 bytes**;
- production direct BirthPolicy creation-code embeds **0**;
- `HELLBOX_ABI_V1` `ReleaseConfig` + `CommitmentSet` structure unchanged.

The permanent actual-factory checkpoint proves the real deployment path and unchanged production sizes. The existing factory payload regression continues to enforce constructor-appended EIP-3860 headroom; do not treat the **29,345-byte creation bytecode** alone as a complete constructor-appended payload measurement.
...
13. the entropy test boundary remains deterministic for unit/fuzz testing, while frozen drand `evmnet` is now consumed by the committed production creator/Prize Wallet FIFO path; collector and timed-closure consumption remain;
14. `HELLBOX_ABI_V1` and the atomic code-store deployment graph remain unchanged.

## 49A.7 Current checkpoint — PRODUCTION PRIZE WALLET SOURCE INTEGRATION COMMITTED

Committed and permanent:

- frozen drand `evmnet` provider/configuration and stateless verifier;
- one equivalent verifier deployed/frozen per factory generation;
- publication validates/stores the factory verifier, provider digest and runtime code hash;
- append-only future-round requests and exact FIFO permissionless proof consumption;
- no timestamp/blockhash/caller fallback, request skipping or Harrow selection;
- internal one-time standard-native Prize Wallet request/fulfillment;
- same 210-candidate pool, normal #066 eligibility, no hard-coded #007 or special traits;
- recipient EOA validation at request and fulfillment;
- permanent eight-test FIFO regression;
- EIP-712 self-authorization by the fresh campaign EOA;
- generation/address/manifest registry with no wallet/manifest reuse;
- explicit generation-bound approval for official publications;
- one publication reserve→complete lifecycle;
...
The registry source was committed separately at `7208590`; its initial test was committed at `c917637`, then extended at `9a153dd` with the three progressive-jackpot regressions. Production `HellboxPublication` integration was committed at `037a723`. It first passed 3/3 temporary focused tests and the then-current 120-test suite. Commit `151d545` added the permanent actual-factory file; 5/5 focused tests and the full 125-test committed suite passed with zero failures or skips.

Exact next production sequence:

1. add the publication-only timed-closure inventory finalizer to `HellboxBirthPolicy.sol` and prove it separately;
2. activate permissionless `TIMED_CLOSURE` through the publication FIFO, selecting Final 3 and extinguishing every other candidate without manual choice;
3. public phases remain closed until Prize Wallet completion and the immutable deadline rules are enforceable;
4. build the separate sale/mint-policy module for phase eligibility, request escrow and exact payment settlement;
5. bind that module through the factory and add only a narrow publication collector-request/fulfillment bridge;
6. bind the separate immutable renderer/data-store path before Gate 4 collector acceptance.

Prize Wallet invariants:

- fresh 12-word MetaMask-compatible EOA generated offline;
- persistent across releases until direct winner confirmation;
- separate Byte-site intro puzzle, not a per-comic puzzle;
- no phrase/private key/final unlock key/answer sheet for Harrow;
- no fixed copy/MARK/DEFECT, reroll, substitution or reset-before-claim;
- no publication-selected recipient;
- pending deposit and provider failures fail closed;
- claim capability is ordinary EOA control and is never an input to publication logic.

This sequence preserves `HELLBOX_ABI_V1`; campaign registry state is factory-generation operational provenance, not a caller-supplied release field.
...
- frozen phase pricing mode + payment asset/contract + exact raw-unit amount;
...
- human descriptions;
- source-art/work files and preview reports;
- rich phase copy;
- protected interactive Reader publication content;
- audit explanations.

Native canonical cover/metadata are **not** in this off-chain-only category. Their required renderer code, art-data bytes and authoritative state must exist on-chain; rich manifests/reports may additionally explain them.
...
production Prize Wallet actual-factory tests      5
cross-language golden vector                      1
--------------------------------------------------
COMMITTED TOTAL                                 125
```

The three progressive-jackpot regressions are committed at `9a153dd`; the five actual-factory integration regressions at `151d545` raise the committed total to 125.
...
### Permanent real-factory production integration proof — COMPLETE

Commit `151d545` permanently proves actual factory campaign activation, official publication deployment/registration and approval, production creator initialization, reserve→seventh-mint→completion accounting, reservation/completion rollback with exact-request retry, receiver-reentrancy rejection and campaign-wallet code-drift failure/retry. The source/test identity is frozen in Project State.

### Native timed closure

- pre-deadline eligible mint succeeds; post-deadline ordinary mint fails;
- no deadline extension/reopen path;
- pending FIFO requests and closure have one deterministic order;
- expiry with `>3` remaining selects unbiased Final 3 and extinguishes all others;
- expiry with exactly `3` remaining awards those three;
- #066 cannot be manually forced;
...
- exact frozen `FREE`, native and ERC-20 phase behavior;
- exact under/overpayment and balance-delta checks;
- wallet lifetime cap survives transfer/burn/phase changes;
- no repricing after `PUBLISH`;
- frozen primary routing boundary;
- collector endpoint cannot bypass prize, randomness, phase, payment or deadline.

### Testnet acceptance

- real PulseChain Testnet V4 factory→publication→mint→ownership→Gate 3→Reader path;
- same reviewed version deploys a second dummy publication without bespoke Solidity;
- progressive campaign events/public display match chain state;
- optimized sizes and full regression remain within limits after every major file.
...
- trait totals;
- each phase accepted payment asset/contract;
- each phase exact raw-unit price;
- phase rules;
- wallet limits;
- royalty bps;
...
- package digest;
- renderer address, runtime code hash, interface/version and art-data store/root bindings;
- renderer rules/version;
- holder-license/rights digest and canonical rights summary;
- capability policy;
- external protocol compatibility binding/class;
- closure policy;
- template/version.

## Fully on-chain canonical artifact proof

For a minted native proving token:

```text
fresh machine / clean-room script
→ read verified publication + renderer + art-data store bytecode/state by RPC
→ call/decode tokenURI(tokenId)
→ no Hellbox API, marketplace, HTTP asset, DNS, CDN or IPFS gateway
→ reconstruct canonical JSON + cover
→ hashes/output match frozen package/renderer/art-data commitments
```

Must prove:
- renderer and art-data references are immutable and code/data hashes match;
- representative maximum output is usable by supported RPC/client/browser/marketplace paths;
- MARK/DEFECT and each implemented artifact-state transition deterministically affect output under frozen rules and the frozen painter order;
- off-chain-only state cannot alter the canonical cover;
- representative SVG contains no executable/remote/user-injected surface;
- any time-derived milestone is checkpointed/evented and Archive stops its covered accumulation;
- ERC-4906-compatible refresh signals fire for canonical metadata changes;
- holder license/rights statement is durable, explicit, independently retrievable and does not falsely equate token ownership with copyright transfer;
- disabling Hellbox's site/API leaves token ownership and canonical art reconstruction intact.

## Prize Wallet proof

```text
fresh offline-generated campaign EOA self-authorizes
→ factory activates generation
→ creator #001–#006 complete
→ approved publication reserves active wallet
→ seventh successful mint requests future-round randomness
→ same-pool FIFO fulfillment mints one random copy
→ publication completes deposit
→ candidate/non-tail state 209/206
→ later publications repeat into the same campaign
→ solver restores EOA and confirms claim
→ fresh generation may activate
```

Must prove:

- result is not hard-coded #007 and #066 can participate;
- Harrow cannot select/reroll/withdraw/claim, and publication cannot choose an arbitrary recipient;
- EIP-712 authorization is factory/generation/manifest/deadline-bound and cannot replay across factories;
- wallet is a fresh no-code EOA and wallet/manifest reuse is impossible;
- two or more distinct publications accumulate one completed deposit each in the same campaign;
- each publication can reserve/complete only once;
- pending deposits block direct winner claim and rotation;
- owner approvals are generation-bound and owner-only;
- successful production fulfillment—not a harness—completes the factory deposit atomically;
- public phases remain closed before completion;
- optional assets carry no guaranteed value and spam transfers are not official;
- campaign-held comics have zero effective Archive reward weight;
- winner claim is a direct call from the recovered EOA, not a public-secret commit/reveal;
- old EOA/manifest/history remain immutable and cannot be reset/reused;
- exact puzzle package/capsule digest stays fixed while completed publication events grow the jackpot;
- no phrase/private key/unlock key/answer sheet is required by chain, source, logs or Harrow tooling.
...
- downstream revenue-routing state may change without mutating any frozen phase payment mode/asset/raw-unit price or issue royalty BPS/deadline;
...
- no current publication field/order prevents a later immutable renderer/data-store context; Gate 4 must add/prove that context before close without rewriting `HELLBOX_ABI_V1`;
- canonical native cover/metadata can remain fully on-chain while protected Reader pages remain a separate committed/continuity-capable package.
...
Prize Wallet campaign manifest (EOA generation/address, activation authorization parameters, puzzle-package + encrypted-capsule digests, terms and official-deposit/disclosure policy; never phrase/key/answer)
...
## G. V1 pricing is configurable per phase before PUBLISH and immutable afterward

Gate 4 V1 supports `FREE`, `FIXED_NATIVE` and `FIXED_ERC20`. PLS is the standard PulseChain paid preset, but each allowlist/early/public/partner phase may select its own free/native/validated-token policy before `PUBLISH`. Every referenced asset and raw-unit amount freezes at deployment; no simultaneous multi-asset or USD-target/oracle conversion path exists.

## H. Revenue routing is deliberately operational downstream of frozen issue economics

The release freezes each phase payment mode, asset, raw-unit price, royalty BPS and the stable routing boundary. Downstream destination wallets, split percentages, reward-token choice and buy/burn/reward strategy remain operationally mutable through the separate gated routing system. Those operational changes must not rewrite supply, rarity, ownership, price, royalty BPS or the native deadline.
...
## N. Solo-operator maintainability, Prize Wallet secret separation and publisher continuity are architecture constraints

## O. A content digest is integrity, not availability

Native Hellbox canonical cover/metadata must be reconstructible from immutable on-chain code/data/state. HTTP/IPFS/Arweave/R2 may mirror it, but no external host/gateway is the canonical dependency.

## P. Token ownership and copyright are not synonyms

The collector owns the ERC-721 and unique artifact state and can independently reconstruct the canonical art. Copyright, trademarks and official-canon authority follow only the frozen holder license/rights statement. Hellbox wins the ownership argument by being technically durable **and** legally precise.

## Q. Gate 4 binds the renderer; Gate 6 builds the art factory

Gate 4 must leave the non-upgradeable publication with a proven immutable renderer interface/binding and self-contained metadata path. Gate 6 may then build the full compositor/data-store compiler without redeploying the ownership model or placing all art in the publication runtime.
...
- [x] Gate 4 V1 pricing is configurable per phase before `PUBLISH`: `FREE`, `FIXED_NATIVE` or `FIXED_ERC20`; PLS is the standard preset and each phase freezes one exact route/amount.
- [x] Every phase payment mode + asset address + exact raw-unit amount and the issue royalty BPS freeze at `PUBLISH`; downstream primary/royalty recipients, split percentages and future reward-token strategy remain operationally mutable.
...
- [x] A digest alone is not sufficient Native Issue canonical-art availability; native cover/metadata must be fully on-chain/reconstructible.
- [x] Native `tokenURI` must be self-contained and use immutable renderer/art-data bindings; the standard `image` field is canonical and no mutable base URI/admin image replacement/external canonical renderer/reachable code-deletion path exists.
- [x] Native cover rendering freezes a deterministic plate/MARK/DEFECT/history/context/Archive-slab painter model, rejects executable/remote/user-injected SVG, and uses checkpointed/evented finite time milestones rather than invisible continuous drift.
- [x] Vector-first art is the self-funded default; raster is allowed only when all compressed bytes are immutable on-chain and deployment/RPC/marketplace budgets pass.
- [x] Token/artifact ownership and copyright/trademark/license scope must be explicitly distinguished in a frozen Artifact Ownership Covenant.
- [x] Storage delivery pointers may migrate only for protected Reader/mirror/derivative objects when committed bytes remain identical; they are not canonical native cover authority.
...
- [x] Prize Wallet seventh-mint/first-non-tail rule and `210/207 → 209/206` math are locked.
- [x] No Harrow prize claim/withdraw/reroll capability or phrase/private key/final unlock key may enter source/config/logs.
...
- [x] Production profile is committed: optimizer enabled, 200 runs, `via_ir = false`.
...
- [x] Factory remains deployable with the committed optimized registry implementation: runtime **12,424 bytes**, margin **12,152 bytes**; initcode with arguments **19,742 bytes**, EIP-3860 headroom **29,410 bytes**; embedded publication creation code remains superseded.
...
- [x] Publication/factory → code store → publication-owned `CREATE` atomic BirthPolicy wiring + immutable companion provenance are implemented/tested; after production Prize Wallet integration, publication runtime is **19,330 bytes** with **5,246 bytes** EIP-170 headroom and creation bytecode is **29,345 bytes** with **19,807 bytes before constructor arguments**. The actual-factory path is permanently tested; the existing payload regression continues to enforce constructor-appended EIP-3860 headroom.
...
- [x] Production creator initialization, actual campaign reservation, seventh-mint FIFO request and atomic factory deposit completion source path are committed at `037a723`; permanent actual-factory composition/adversarial regression is committed at `151d545`; 5/5 focused and 125/125 total tests passed.
...
- [x] Committed test inventory = **125** including 8 permanent FIFO tests, 15 permanent registry/progressive tests and 5 permanent actual-factory integration tests; creator run = **125 passed / 0 failed / 0 skipped**; issuance fuzz boundary remains **256 runs**.
...
After this synchronization, the next frontier is **BirthPolicy timed-closure inventory finalization, then publication FIFO timed closure**. The selected provider, verifier, factory binding, production reserve/request/complete source path, permanent actual-factory proof, trait-consumption and atomic code-store graphs are already tested and must be preserved. Then implement the separate sale/mint-policy module and narrow publication collector bridge, followed by the lean immutable metadata-renderer binding/self-contained `tokenURI` proving path.
